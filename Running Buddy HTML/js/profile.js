...
function updateRunnerProfile(runner) {
    ...
    if (distanceElement) {
        ...
        const selectedRunner =
            runners.find((r) => r.id === selectedRunnerId) || runners[0];
    ...
    const totalRunsElement = document.querySelector(
    ...
    const totalDistanceElement = document.querySelectorAll(
        ".activity-stat-card .activity-stat-value"
    )[1];
    ...
    const bestPaceElement = document.querySelectorAll(
        ".activity-stat-card .activity-stat-value"
    )[2];
...
function shareProfile() {
    const runnerId = sessionStorage.getItem("selectedRunner");
    const runnerName =
        document.getElementById("profile-name")?.textContent || "Runner";
...
function showQuickMessageModal(runnerName) {
    ...
    const quickList = QUICK_MESSAGES.map(
        (m, idx) => `
